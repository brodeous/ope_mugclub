...
class MemberInfoConfirmPage extends StatefulWidget {
    ...
    const MemberInfoConfirmPage({super.key, this.first, this.last, this.email, this.phone, this.qrCode});
...
class _MemberInfoConfirmPageState extends State<MemberInfoConfirmPage> {
                      ...
                      children: <Widget>[
                          ...
                          ElevatedButton(
                              onPressed: () {
                                ...
                                memberInfoRef.child('${widget.qrCode}/').set(
                                    newUser.toDict()
                                )
                                .then((_) => debugPrint('Member\'s info written'))
                                .catchError((error) => debugPrint('Error writting to Database! $error'));
                              }, 
    ...
    void _fixString() {
       ...
       String first = '${widget.first?[0].toUpperCase()}${widget.first?.substring(1,widget.first?.length)}';
       String last = '${widget.last?[0].toUpperCase()}${widget.last?.substring(1,widget.last?.length)}';
       String email = '${widget.email}';
       String phone = '${phoneTMP.substring(0,3)}-${phoneTMP.substring(3,6)}-${phoneTMP.substring(6,10)}';
